...
public class Firework{
    ...
    public Firework() {
    	...
    	for (int x = 0; x < sparks.length; x++) {
    		sparks[x] = new Spark(FireworkDisplay.WIDTH/2, FireworkDisplay.HEIGHT);
    	}
    }
    ...
    public void launch() {
    	...
    	for (Spark x : sparks) {
    		x.x = FireworkDisplay.WIDTH/2;
    		x.y = FireworkDisplay.HEIGHT;
    	}
    ...
    public void drawSparks(Graphics g) {
    	...
    	for (int x = 0; x < sparks.length; x++) {
    		g.setColor(sparks[x].color);
    		g.fillOval(sparks[x].x,sparks[x].y, sparks[x].size, sparks[x].size );
    	}
